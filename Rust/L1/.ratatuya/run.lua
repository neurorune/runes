...
local DIR = "."
local BIN_DIR = ".ratatuya/bin"
local COMPILER = "rustc"
local FLAGS = "-C debuginfo=2"

os.execute("mkdir -p " .. BIN_DIR)

print("👀 Watching Rust saves, deletes, renames…")
...
for line in watcher:lines() do
  ...
  if not filename or not filename:match("%.rs$") then
    goto continue
  end

  local base = filename:gsub("%.rs$", "")
  local bin = BIN_DIR .. "/" .. base

  -- 🗑️ DELETE
  if events:match("DELETE") or events:match("DELETE_SELF") then
    os.execute("rm -f " .. bin)
    print("🗑️  Removed binary: " .. bin)
  ...
  elseif events:match("MOVED_TO") then
    ...
    if old then
      local old_bin = BIN_DIR .. "/" .. old:gsub("%.rs$", "")
      local new_bin = BIN_DIR .. "/" .. filename:gsub("%.rs$", "")

      if os.execute("[ -f " .. old_bin .. " ]") then
        os.execute("mv " .. old_bin .. " " .. new_bin)
        print("🔁 Renamed binary: " .. old_bin .. " → " .. new_bin)
      end
