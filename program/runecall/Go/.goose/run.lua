...
print("👀 Watching Go saves, deletes, renames…")
print("🚀 Runs with `go run` → output shows on screen")
...
for line in watcher:lines() do
  ...
  if not filename or not filename:match("%.go$") then
    goto continue
  end
  ...
  elseif events:match("CLOSE_WRITE") then
    ...
    local code = os.execute(string.format("%s run %s", GO, filename))
