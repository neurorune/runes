...
local DIR = "."
local BIN_DIR = ".croc/bin"
local COMPILER = "gcc"
local FLAGS = "-Wall -Wextra -O0 -g"

os.execute("mkdir -p " .. BIN_DIR)

print("👀 Watching C saves, deletes, renames…")
...
for line in watcher:lines() do
  ...
  if not filename or not filename:match("%.c$") then
    goto continue
  end

  local base = filename:gsub("%.c$", "")
  ...
  elseif events:match("MOVED_TO") then
    ...
    if old then
      local old_bin = BIN_DIR .. "/" .. old:gsub("%.c$", "")
      local new_bin = BIN_DIR .. "/" .. filename:gsub("%.c$", "")
